...
MAPR_VERSION_CORE=${MAPR_VERSION_CORE:-6.0.0}
MAPR_VERSION_MEP=${MAPR_VERSION_MEP:-4.0.0}
...
declare -a SUPPORTED_RELEASES_SUSE=('11.3' '12' '12.0' '12.1' '12.2')
...
fetch_installer_suse() {
    ...
    if [ -n "$MAPR_INSTALLER_PACKAGES" ]; then
        zypper --non-interactive -q --no-gpg-checks install -n $MAPR_INSTALLER_PACKAGES
    elif [ $ISCONNECTED -eq $YES ]; then
        # Create the mapr-installer repository information file
...
repo_add_ubuntu() {
    ...
    apt-key adv -q $options --fetch-keys $MAPR_GPG_KEY_URL || repo_add_error
    cat > $1 << EOM
deb $2
EOM
    apt-get update -qq
...
setup_service_cmd() {
    ...
    if [ $USE_SYSTEMCTL -eq $YES ]; then
        systemctl $1 $2
        return
    elif [ -f $svc_file1 -o -f $svc_file2 ]; then
...
docker_client() {
    ...
    if [ $ANSWER -eq $YES ]; then
        ...
        [ $ANSWER -eq $YES -a $mep_version_major -gt 2 ] && PACKAGES="$PACKAGES mapr-librdkafka"
...
installer_check_update() {
    ...
    [ $ISUPDATE -eq $NO -a $defs -eq $YES ] && installer_remove "silent"
...
[ "$1" != "docker" -a $ID -ne 0 ] && msg_err "$CMD must be run as 'root'"
